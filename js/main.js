...
// All content comes from data/portfolio.json so it can work on GitHub Pages and Netlify.
...
const defaultPortfolioData = {
  site: {
    logoText: "Fatma.dev",
    pageTitle:
      "Fatma Sorour | Full-Stack Developer Portfolio - ASP.NET Core & Angular Expert",
    metaDescription:
      "Fatma Sorour - Professional Full-Stack Developer specializing in ASP.NET Core, Angular, MSSQL Server, and enterprise web applications.",
    canonicalUrl: "https://your-domain.netlify.app/",
  },
  personal: {
    name: "Fatma Sorour",
    fullName: "Fatma Mohamed Sorour",
    typewriterText: "Fatma Sorour...",
    title: "Full-Stack Developer",
    heroDescription:
      "Specialized in ASP.NET Core, Angular, MSSQL Server, and enterprise web applications. Passionate about building scalable, secure, and production-ready systems with clean architecture and modern UI/UX.",
  ...
  socialLinks: [
    {
      platform: "github",
      url: "https://github.com/FatmaSorour8",
      icon: "fab fa-github",
      hoverClass: "hover:text-blue-400",
    },
    {
      platform: "linkedin",
      url: "https://www.linkedin.com/in/fatma-mohamed-sorour/",
      icon: "fab fa-linkedin",
      hoverClass: "hover:text-blue-400",
    },
    {
      platform: "whatsapp",
      url: "https://wa.me/201288427468",
...
async function loadPortfolioData() {
  try {
    ...
    if (!response.ok) {
      throw new Error("Cannot load portfolio.json");
    }

    portfolioDataCache = await response.json();
  } catch (error) {
    console.warn(
      "Using fallback portfolio content because JSON could not be loaded.",
      error
    );
...
function assetUrl(url, fallback = "#") {
  const value = safeUrl(url, fallback);
    ...
    .map((link) => {
      ...
      return `
        <a href="${safeUrl(link.url)}"
           ...
           class="${hover} text-xl transition transform hover:scale-110"
...
function applyPortfolioData(data) {
  ...
  document.title =
    site.pageTitle || `${personal.name || "Portfolio"} | Portfolio`;

  document
    .querySelector('meta[name="description"]')
    ?.setAttribute(
      "content",
      site.metaDescription || personal.heroDescription || ""
    );

  document
    .querySelector('meta[name="author"]')
    ?.setAttribute("content", personal.fullName || personal.name || "");
  ...
  document
    ...
    ?.setAttribute("href", site.canonicalUrl || window.location.href);

  const logo = document.querySelector("nav a.gradient-text");
  if (logo) {
    logo.textContent = site.logoText || `${personal.name || "Portfolio"}.dev`;
  }
  ...
  if (heroDescription) {
    heroDescription.textContent = personal.heroDescription || "";
  }

  const profileImg = document.querySelector(".profile-img");
  if (profileImg) {
    profileImg.src = assetUrl(personal.profileImage, "assets/profile.png");
    profileImg.alt = `${personal.name || "Profile"} - Profile Picture`;
  }

  document
    .querySelectorAll('a[id^="downloadCvBtn"], a[href*="Fatma_Sorour_CV"]')
    .forEach((a) => {
      a.href = assetUrl(personal.cvFile, "assets/Fatma_Sorour_CV.pdf");
      a.setAttribute(
        "download",
        (personal.cvFile || "Fatma_Sorour_CV.pdf").split("/").pop()
      );
  ...
  if (aboutText) {
    aboutText.innerHTML = (personal.aboutDescription || [])
      .map((p) => `<p>${escapeHtml(p)}</p>`)
      .join("");
  }

  renderSocialLinks(
    document.querySelector("#home .flex.gap-6.pt-6"),
    data.socialLinks || []
  );

  renderSocialLinks(
    document.querySelector("#contact .flex.gap-6.pt-12"),
    data.socialLinks || []
  );

  renderSocialLinks(
    document.querySelector("footer .flex.items-center.gap-6"),
    data.socialLinks || []
  );
  ...
  function typeWriter() {
    if (index < text.length) {
      typewriterElement.innerHTML =
        escapeHtml(text.substring(0, index + 1)) +
      ...
      typewriterElement.innerHTML =
        escapeHtml(text) + '<span class="typewriter-cursor"></span>';
...
function renderSkills(skills) {
  const renderBadges = (selector, list = []) => {
    ...
    el.innerHTML = list
      .map(
        (s) =>
          `<span class="skill-badge px-4 py-2 rounded-full text-sm font-medium inline-block transition">${escapeHtml(
            s
          )}</span>`
      )
      .join("");
...
function renderEducation(items = []) {
  ...
  container.innerHTML = items
    .map(
      (edu, idx) => `
      <div class="p-6 rounded-2xl border-l-2 border-blue-500 transition transform hover:translate-x-1 stagger-item delay-${
        ((idx % 5) + 1) * 100
...
function renderExperiences(items = []) {
  ...
  container.innerHTML = items
    .map((exp, idx) => {
      ...
      return `
        <div class="ml-5 relative timeline-item stagger-item delay-${
          ((idx % 5) + 2) * 100
        }">
          ...
          <div class="glass-card p-5">
            <div class="flex flex-wrap justify-between items-start gap-4">
              <div>
                <h3 class="text-2xl font-bold text-${color}-300">${escapeHtml(
        exp.title
      )}</h3>
                <p class="text-lg font-medium mt-1">${escapeHtml(
                  exp.company
                )}</p>
                <p class="text-sm mt-1 text-gray-400">${escapeHtml(
                  exp.location
                )}</p>
              </div>
  ...
  container.innerHTML = `
    ...
    <div class="contact-info-item">
      ...
      <a href="mailto:${escapeHtml(
        email
      )}" class="hover:text-blue-400 transition">${escapeHtml(email)}</a>
    </div>
    ...
    <div class="contact-info-item">
      ...
      ${escapeHtml(info.availability || "")}
    </div>
  `;

  const footer = document.querySelector("footer p");
  if (footer) {
    footer.innerHTML = `© <span id="currentYear">${new Date().getFullYear()}</span> ${escapeHtml(
      portfolioDataCache?.personal?.name || ""
    )} — ${escapeHtml(
      portfolioDataCache?.personal?.title || ""
    )}. All rights reserved.`;
  }
}
...
function renderProjects(page = 1) {
  ...
  const itemsPerPage = Number(
    portfolioDataCache.projectsSettings?.itemsPerPage || 3
  );
  ...
  container.innerHTML = currentProjects
    .map((proj, idx) => {
      const imgSrc = assetUrl(
        proj.image,
        `https://placehold.co/600x400/1e293b/3b82f6?text=${encodeURIComponent(
          proj.title || "Project"
        )}`
      );
      ...
      return `
        <div class="glass-card p-0 overflow-hidden h-full transition-all duration-300 project-card stagger-item ${delayClass} flex flex-col revealed">
          <div class="relative w-full h-48 overflow-hidden bg-gray-800 group">
            <img src="${imgSrc}"
                 alt="${escapeHtml(proj.title)} screenshot"
                 class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                 onerror="this.onerror=null; this.src='https://placehold.co/600x400/1e293b/3b82f6?text=${encodeURIComponent(
                   proj.title || "Project"
                 )}';">
                ...
                ? `
                  <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-start p-4">
                    <a href="${safeUrl(liveUrl)}"
                       target="_blank"
                       rel="noopener noreferrer"
                       class="text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-full text-xs font-semibold shadow-lg transition transform hover:scale-105 flex items-center gap-1">
          ...
          <div class="p-5 flex flex-col flex-grow">
            ...
            <p class="text-sm mt-2 flex-grow" style="color:var(--text-secondary)">
            ...
            <div class="flex flex-wrap gap-2 mt-4">
              ${(proj.technologies || [])
                .map(
                  (t) =>
                    `<span class="project-tag text-xs inline-block transition">${escapeHtml(
                      t
                    )}</span>`
                )
                .join("")}
            ...
            ${
              githubUrl || liveUrl
                ? `
                  <div class="mt-5 flex gap-4 pt-3 border-t border-gray-700/40">
                    ${projectLink(
                      githubUrl,
                      "text-xs text-blue-400 hover:text-blue-300 transition flex items-center gap-1",
                      "fab fa-github",
                      "Source Code"
                    )}
                    ...
                    ${projectLink(
                      liveUrl,
                      "text-xs text-purple-400 hover:text-purple-300 transition flex items-center gap-1",
                      "fas fa-globe",
                      "Live Preview"
                    )}
                  </div>
...
function renderProjectPagination(totalPages, totalItems, itemsPerPage) {
  ...
  buttons.push(`
    <button class="pagination-btn"
            ${currentProjectsPage === 1 ? "disabled" : ""}
            data-page="${currentProjectsPage - 1}"
  ...
  buttons.push(`
    <button class="pagination-btn"
            ${currentProjectsPage === totalPages ? "disabled" : ""}
            data-page="${currentProjectsPage + 1}"
  ...
  buttons.push(`
    <span class="pagination-info">
      Page ${currentProjectsPage} of ${totalPages}
    </span>
  `);
...
function updateToggleIcons(theme) {
  ...
  document
    .querySelectorAll("#desktopThemeToggle i, #sideThemeToggle i")
    .forEach((icon) => {
...
function revealNewItems() {
  document
    .querySelectorAll("section, .timeline-item, .glass-card:not(.stagger-item)")
    .forEach((el) => {
    ...
    for (let b = a; b < particlesArray.length; b++) {
      ...
      if (distance < maxDistance * maxDistance) {
        ...
        ctx.strokeStyle = isLightMode
          ? `rgba(37, 99, 235, ${opacityValue * 0.5})`
          : `rgba(167, 139, 250, ${opacityValue * 0.2})`;
